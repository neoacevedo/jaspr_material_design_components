...
class _AppState extends State<App> {
  ...
  Map<String, String> menuAttributes = {'id': 'usage-menu2', 'anchor': 'usage-anchor2'};

  // Variable Key para identificar al componente Dialog.
  final GlobalNodeKey<web.HTMLFormElement> myDialogKey = GlobalNodeKey();

  // Variable Key para identificar al componente Menu.
  final GlobalNodeKey<web.HTMLFormElement> menuKey = GlobalNodeKey();

  final GlobalNodeKey<web.HTMLElement> tabKey = GlobalNodeKey();

  // Variables para manejar el estado de los tabs.
  int activeTabIndex = 0; // Variable para almacenar el panel actual visible.
  int previousTabIndex = 0; // Variable para almacenar el índice del tab anterior.

  web.HTMLElement? currentPanel;
          ...
          Card.outlined(
            children: [
              ...
              div([
                ...
                Radio(attributes: {'name': 'animals', 'value': 'birds', 'checked': ''}),
              ]),
            ],
            cssClass: 'group',
          ),
          ...
          Card(
            children: [
              ...
              Tabs(
                ...
                events: {
                  'change': (event) {
                    web.Element? root = tabKey.currentNode?.getRootNode() as web.Element?;
                    ...
                    Future.microtask(() {
                      ...
                      if (previousTabIndex != activeTabIndex) {
                        var previousPanelId = tabKey.currentNode?.children.item(previousTabIndex)?.getAttribute('aria-controls');
                        currentPanel = root?.querySelector('#$previousPanelId') as web.HTMLElement?;
                      ...
                      var panelId = tabKey.currentNode?.children.item(activeTabIndex)?.getAttribute('aria-controls');

                      currentPanel = root?.querySelector('#$panelId') as web.HTMLElement?;
